...
import { fetchConvertAreaLogs } from "../services";
import type { PosConvertHistory } from "../types";

const ROWS_PER_PAGE = 20;

export default function ConvertAreaLogsPage() {
    const [logs, setLogs] = useState<PosConvertHistory[]>([]);
    ...
    return (
        <div className="flex flex-col gap-6">
            {/* HEADER */}
            <div>
                <h1 className="text-2xl font-bold text-gray-800">Convert Area Logs</h1>
                <p className="mt-1 text-sm text-gray-500">
                    History of all area changes for POS devices.
                </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                <div className="flex items-center gap-2">
                    ...
                    <input
                        ...
                        className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 focus:border-teal focus:outline-none focus:ring-2 focus:ring-teal/20 transition-all"
                    />
                </div>
                <div className="flex items-center gap-2">
                ...
                {(dateFrom || dateTo) && (
                    <button
                        ...
                        className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-all"
                    >
                        Clear Filters
                    </button>
                )}
                    ...
                    <table className="min-w-full text-sm">
                        <thead>
                            <tr className="border-b border-gray-100 bg-gray-50/80">
                                ...
                                <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Previous Area</th>
                                <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">New Area</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredLogs.length === 0 ? (
                                ...
                                paginatedLogs.map((log) => {
                                    const from = log.previous_area || "N/A";
                                    const to = log.new_area || "N/A";
                                    return (
                                        <tr key={log.id} className="transition-colors hover:bg-teal-50/40">
                                            ...
                                            <td className="px-5 py-4 text-gray-700">
                                                <span className="inline-flex items-center gap-1.5 rounded-md bg-teal-50 px-2.5 py-1 font-medium text-teal-700">
                                                    {to}
                                                </span>
                                            </td>
                                            <td className="px-5 py-4">
