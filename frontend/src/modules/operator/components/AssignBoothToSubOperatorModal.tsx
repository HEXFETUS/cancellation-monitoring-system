...
export default function AssignBoothToSubOperatorModal({
    open,
    booth,
    operators = [],
    ...
    const [existingPending, setExistingPending] = useState<BoothOperatorChangeRequest | null>(null);
    ...
    // Check for existing pending booth operator change requests for this booth
    useEffect(() => {
        if (open && booth) {
            listBoothOperatorChangeRequests({
                boothId: booth.id,
    ...
    const availableSubOperators = operators.filter((op) => {
        // Exclude the current operator assigned to this booth
        if (booth.operator_id != null && Number(op.id) === Number(booth.operator_id)) return false;
    ...
    const submit = async () => {
        ...
        try {
            ...
            await createBoothOperatorChangeRequest({
                ...
                booth_id: booth.id,
                reason: reason.trim(),
                old_operator: booth.operator || null,
            });
            await onSubmitted();
        } catch (e) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 backdrop-blur-sm px-4 pt-16 sm:pt-24">
            <div className="w-full max-w-md animate-in fade-in zoom-in-95 duration-200 rounded-2xl bg-white shadow-2xl border border-warm overflow-hidden">
                ...
                <div className="p-6">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-lg font-bold text-ink">Assign Outlet to Sub-Operator</h2>
                            <p className="text-sm text-ink-muted mt-0.5">Transfer an outlet to a sub-operator</p>
                        </div>
                        <button
                            onClick={onClose}
                            disabled={saving}
                    ...
                    <div className="space-y-4">
                        {existingPending && (
                            <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700">
                                This outlet already has a pending operator change request. Please wait for it to be processed.
                            </div>
                        )}
                        ...
                        <div className="space-y-1 rounded-lg border border-warm bg-cream/50 px-3 py-2 text-sm">
                            <div className="text-ink">
                                <span className="font-medium">Outlet:</span> {booth.booth_code}
                            </div>
                            <div className="text-ink">
                            ...
                            <div className="text-ink">
                                <span className="font-medium">Current Operator:</span> {booth.operator || "—"}
                            </div>
                        </div>
                        ...
                        <label className="block">
                            ...
                            {availableSubOperators.length === 0 ? (
                                <div className="rounded-lg border border-warm bg-cream/50 px-3 py-2.5 text-sm text-ink-muted">
                                    {mainOperatorId
                                        ? `No available sub-operators assigned to you.`
                                        : "No operator found for this outlet."}
                                </div>
                            ) : (
                                <Dropdown
                                    value={subOperatorId}
                                    placeholder="Select a sub-operator"
                                    emptyMessage="No sub-operators available."
                        ...
                        <label className="block">
                            ...
                            <textarea
                                ...
                                rows={3}
                                placeholder="Explain why you need to assign this outlet to a sub-operator."
            ...
            <ConfirmationModal
                open={showConfirm}
                title="Confirm Outlet Assignment"
                message="Are you sure you want to submit this outlet assignment request?"
            ...
            >
                <div className="flex items-center justify-between px-4 py-2.5">
                    <span className="text-xs font-medium text-ink-muted uppercase tracking-wider">Outlet</span>
                    <span className="text-sm font-semibold text-ink">{booth.booth_code}</span>
                </div>
                <div className="flex items-center justify-between px-4 py-2.5">
                    <span className="text-xs font-medium text-ink-muted uppercase tracking-wider">From Operator</span>
                    <span className="text-sm font-semibold text-ink">{booth.operator || "—"}</span>
                </div>
                <div className="flex items-center justify-between px-4 py-2.5">
