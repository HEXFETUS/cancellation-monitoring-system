...
interface Props {
    ...
    userId: number | null;
    onCancelled?: (request: OperatorChangeRequest) => void;
    onCancelError?: (message: string) => void;
}

export default function OperatorChangeRequestHistory({
    requests,
    search = "",
    onChanged,
    userId,
    ...
    const [cancelTarget, setCancelTarget] = useState<OperatorChangeRequest | null>(null);
    ...
    const handleCancelRequest = async () => {
        ...
        try {
            await cancelOperatorChangeRequest(target.id, userId);
...
function StatusPill({ status }: { status: OperatorChangeRequest["status"] }) {
    const normalizedStatus = ((status || "") as string).toLowerCase() as OperatorChangeRequest["status"];
    const colorMap: Record<OperatorChangeRequest["status"], string> = {
