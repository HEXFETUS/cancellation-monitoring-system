...
export default function AssignCpToSubOperatorModal({
    open,
    cellphone,
    operators = [],
    ...
    const [existingPending, setExistingPending] = useState<CpOperatorChangeRequest | null>(null);
    ...
    // Check for existing pending operator change requests for this cellphone
    useEffect(() => {
        if (open && cellphone) {
            listCpOperatorChangeRequests({
    ...
    const availableSubOperators = operators.filter((op) => {
        if (currentOperatorId != null && Number(op.id) === Number(currentOperatorId)) return false;
    ...
    const submit = async () => {
        ...
        try {
            ...
            await createCpOperatorChangeRequest({
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 backdrop-blur-sm px-4 pt-16 sm:pt-24">
            <div className="w-full max-w-md animate-in fade-in zoom-in-95 duration-200 rounded-2xl bg-white shadow-2xl border border-warm overflow-hidden">
                ...
                <div className="p-6">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-lg font-bold text-ink">Assign CP to Sub-Operator</h2>
                            <p className="text-sm text-ink-muted mt-0.5">Transfer a cellphone to a sub-operator</p>
                        </div>
                        <button
                            onClick={onClose}
                            disabled={saving}
                    ...
                    <div className="space-y-4">
                        {existingPending && (
                            <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700">
                                This cellphone already has a pending operator change request. Please wait for it to be processed.
                            </div>
                        )}
                        ...
                        <div className="space-y-1 rounded-lg border border-warm bg-cream/50 px-3 py-2 text-sm">
                            ...
                            <div className="text-ink">
                                <span className="font-medium">Serial:</span> {cellphone.serial_number}
                            </div>
                            <div className="text-ink">
                        ...
                        <label className="block">
                            ...
                            <textarea
                                ...
                                rows={3}
                                placeholder="Explain why you need to assign this cellphone to a sub-operator."
            ...
            >
                <div className="flex items-center justify-between px-4 py-2.5">
                    ...
                    <span className="text-sm font-semibold text-ink">{cellphone.brand} {cellphone.model}</span>
                </div>
                <div className="flex items-center justify-between px-4 py-2.5">
                    <span className="text-xs font-medium text-ink-muted uppercase tracking-wider">Serial No.</span>
                    <span className="text-sm font-semibold text-ink">{cellphone.serial_number}</span>
                </div>
                <div className="flex items-center justify-between px-4 py-2.5">
                    <span className="text-xs font-medium text-ink-muted uppercase tracking-wider">Control No.</span>
                    <span className="text-sm font-semibold text-ink">{cellphone.control_no}</span>
                </div>
                <div className="flex items-center justify-between px-4 py-2.5">
