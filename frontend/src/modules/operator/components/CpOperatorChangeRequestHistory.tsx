...
interface Props {
    requests: CpOperatorChangeRequest[];
    search?: string;
    onChanged: () => Promise<void>;
    userId: number | null;
    onCancelled?: (request: CpOperatorChangeRequest) => void;
    onCancelError?: (message: string) => void;
}

export default function CpOperatorChangeRequestHistory({
    requests,
    search = "",
    onChanged,
    userId: _userId,
    ...
    const [cancelTarget, setCancelTarget] = useState<CpOperatorChangeRequest | null>(null);
    const [canceling, setCanceling] = useState(false);
    const [cancelError, setCancelError] = useState("");

    const handleCancelRequest = async () => {
        if (!cancelTarget) {
            setCancelError("Unable to cancel: no request selected.");
            return;
        }
        setCancelError("");
        setCanceling(true);
        ...
        try {
            // Use the request's own user_id since that's who the record belongs to
            const cancelUserId = target.user_id;
            const result = await cancelCpOperatorChangeRequest(target.id, cancelUserId);
            if (!result) {
    ...
    const filteredRequests = useMemo(() => {
        ...
        return requests.filter((r) => {
            const controlNo = String(r.control_no || "").toLowerCase();
            const serial = String(r.serial_number || "").toLowerCase();
            return controlNo.includes(q) || serial.includes(q);
        });
    }, [requests, search]);
    ...
    if (requests.length === 0) {
        return (
            <p className="rounded-lg border border-dashed border-[#92C7CF]/20 bg-white/30 px-3 py-4 text-center text-sm text-gray-500">
                You haven't submitted any sub-operator assignment requests yet.
            </p>
        );
    }
    ...
    return (
        <>
                        ...
                        {filteredRequests.length === 0 ? (
                            ...
                            filteredRequests.map((r) => (
                                <tr key={r.id} className="border-b border-white/30 transition hover:bg-[#92C7CF]/8">
                                    ...
                                    <td className="whitespace-nowrap px-4 py-3 text-gray-500">{r.from_operator || r.old_operator || "—"}</td>
                                    <td className="whitespace-nowrap px-4 py-3 text-gray-700">{r.to_operator || "—"}</td>
            ...
            <ConfirmationModal
                ...
                title="Cancel Operator Assignment Request"
                message="Are you sure you want to cancel this pending sub-operator assignment request?"
                ...
                {cancelTarget && (
                    <div className="space-y-2 px-4 py-3 text-sm text-gray-700">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <span>{cancelTarget.from_operator || cancelTarget.old_operator || "—"}</span>
                        </div>
                        <div className="flex items-center justify-between">
                            <span className="font-semibold">To</span>
                            <span>{cancelTarget.to_operator || "—"}</span>
                        </div>
                    </div>
                )}
            </ConfirmationModal>
...
function StatusPill({ status }: { status: CpOperatorChangeRequest["status"] }) {
    const normalizedStatus = ((status || "") as string).toLowerCase() as CpOperatorChangeRequest["status"];
    const colorMap: Record<CpOperatorChangeRequest["status"], string> = {
