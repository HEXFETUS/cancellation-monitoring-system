...
import { CheckCircle2, XCircle, Search, RefreshCw, MapPin, CalendarDays } from "lucide-react";
...
export default function AssignOutletPage() {
    ...
    const load = useCallback(async () => {
        try {
            setLoading(true);
            setError("");
            const data = await listBoothOperatorChangeRequests().catch(() => [] as BoothOperatorChangeRequest[]);
            setRequests(data);
        } catch (err) {
            ...
            >
                ...
                <div className="flex flex-wrap justify-end items-center gap-3 pb-2 xl:pb-1">
                    ...
                    <div className="relative w-full sm:w-64">
                        ...
                        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search booth, operator, location..." className="h-9 w-full rounded-lg pl-8 pr-3 text-sm outline-none transition-all duration-200 focus:border-teal/60 focus:ring-2 focus:ring-teal/35 placeholder:text-gray-400 dark:placeholder:text-gray-400" style={inputStyle} />
                    </div>
                    <button
                        onClick={load}
                        disabled={loading}
