...
export default function AssignPosPage() {
    ...
    const [approveTarget, setApproveTarget] = useState<OperatorChangeRequest | null>(null);
    const [rejectNoteTarget, setRejectNoteTarget] = useState<OperatorChangeRequest | null>(null);
    const [rejectNote, setRejectNote] = useState("");

    const openApprove = (req: OperatorChangeRequest) => setApproveTarget(req);
    const closeApprove = () => setApproveTarget(null);

    const openReject = (req: OperatorChangeRequest) => {
        ...
        <div className="w-full max-w-full space-y-5">
            ...
            >
                ...
                <div className="flex flex-wrap justify-end items-center gap-3 pb-2 xl:pb-1">
                    ...
                    <div className="relative w-full sm:w-64">
                        ...
                        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search device, serial, operator..." className="h-9 w-full rounded-lg pl-8 pr-3 text-sm outline-none transition-all duration-200 focus:border-teal/60 focus:ring-2 focus:ring-teal/35 placeholder:text-gray-400 dark:placeholder:text-gray-400" style={inputStyle} />
                    </div>
                    <button
                        onClick={load}
                        disabled={loading}
            ...
            <div className="space-y-3">
                {loading ? (
                    ...
                    <div className="rounded-xl border border-gray-200 bg-white px-3 py-10 text-center text-sm text-gray-500 shadow-sm">
                        No assign-POS requests match the current filter.
                    </div>
                ) : (
                    paginated.map((r) => {
                        ...
                        return (
                            ...
                            >
                                <div className="flex flex-wrap items-start justify-between gap-3">
                                    <div>
                                        <div className="flex items-center gap-2">
                                            <span className="text-base font-semibold text-gray-800">{deviceLabel}</span>
                                ...
                                <div className="mt-3 grid gap-2 rounded-lg bg-gray-50 p-3 text-sm sm:grid-cols-4">
                                    <div>
                                        <div className="text-xs font-semibold uppercase tracking-wide text-gray-500">Area / Booth</div>
                                        <div className="text-sm text-gray-700">{r.area || "—"} <span className="text-xs text-gray-400">({r.current_booth_code || "—"})</span></div>
                                    </div>
                                    <div>
                    ...
                    {rejectNoteTarget && (
                        <div className="rounded-lg bg-gray-50 p-3 text-sm space-y-1">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="font-semibold text-gray-800">{rejectNoteTarget.device_no || `POS #${rejectNoteTarget.pos_record_id}`}</span>
                            </div>
                            <div className="flex items-center justify-between">
